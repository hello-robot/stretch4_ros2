...
class HeadCalibrator:
    ...
    def __init__(self, node, uncalibrated_urdf_filename, calibration_directory, sample_selector_func, calibration_options, visualize, tilt_angle_backlash_transition_rad):
        ...
        self.telescoping_joints = ['arm_l0_joint', 'arm_l1_joint', 'arm_l2_joint', 'arm_l3_joint', 'arm_l4_joint']
        ...
        self.camera_tilt_joint = self.camera_chain.get_joint_by_name('head_tilt_joint')
        ...
        self.camera_pan_joint = self.camera_chain.get_joint_by_name('head_pan_joint')
        ...
        self.wrist_chain = ca.Chain(self.new_urdf, 'base_link', 'arm_l0_link')

        if self.calibrate_lift:
            # Calibrating the following joint ('mast_joint') results
            ...
            # solutions as calibrating 'lift_joint'. With constraints,
            # the possible solutions can differ due to the mast also
            # influencing the position and orientation of the head
            # assembly.
            self.lift_joint = self.wrist_chain.get_joint_by_name('mast_joint')            
        ...
        if self.calibrate_arm:
            ...
            self.arm_joint = self.wrist_chain.get_joint_by_name('arm_l4_joint')
        ...
        self.error_measures.append(ca.ArucoError('wrist_top', 'wrist', 'aruco_top_wrist_link', aruco_urdf, self.meters_per_deg, rgba))
        rgba = [0.0, 0.0, 1.0, 0.2]
        self.error_measures.append(ca.ArucoError('wrist_inside', 'wrist', 'aruco_inner_wrist_link', aruco_urdf, self.meters_per_deg, rgba))
        rgba = [1.0, 1.0, 0.0, 0.2]
        self.error_measures.append(ca.ArucoError('shoulder', 'shoulder', 'aruco_shoulder_link', aruco_urdf, self.meters_per_deg, rgba))
        rgba = [1.0, 0.0, 0.0, 0.2]
        self.error_measures.append(ca.ArucoError('base_left', 'base', 'aruco_left_base_link', aruco_urdf, self.meters_per_deg, rgba))
        rgba = [1.0, 0.0, 1.0, 0.2]
        self.error_measures.append(ca.ArucoError('base_right', 'base', 'aruco_right_base_link', aruco_urdf, self.meters_per_deg, rgba))
    ...
    def update_sample_with_offsets_and_backlash(self, sample):
        ...
        joints['head_pan_joint'] = joints['head_pan_joint'] + self.pan_angle_offset + pan_backlash_correction
        joints['head_tilt_joint'] = joints['head_tilt_joint'] + self.tilt_angle_offset + tilt_backlash_correction
