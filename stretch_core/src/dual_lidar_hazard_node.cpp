...
class DualLidarHazardNode : public rclcpp::Node
{
public:
  DualLidarHazardNode()
  : Node("dual_lidar_hazard"),
    ...
    tf_listener_(tf_buffer_)
  {
    ...
    param_callback_handle_ = add_on_set_parameters_callback(
      std::bind(&DualLidarHazardNode::onParameterChange, this, std::placeholders::_1));
  }
  ...
  void activateSubscription()
  {
    ...
    sub1_ = create_subscription<sensor_msgs::msg::PointCloud2>(
      lidar1_topic_, qos,
      std::bind(&DualLidarHazardNode::pointcloudCallback1, this, std::placeholders::_1));
    sub2_ = create_subscription<sensor_msgs::msg::PointCloud2>(
      lidar2_topic_, qos,
      std::bind(&DualLidarHazardNode::pointcloudCallback2, this, std::placeholders::_1));
    timer_ = create_wall_timer(
      std::chrono::milliseconds(100),
      std::bind(&DualLidarHazardNode::timerCallback, this));
  }
...
private:
  void declareParameters()
  {
    ...
    declare_parameter("pointcloud_z_min", std::numeric_limits<double>::quiet_NaN());
    declare_parameter("pointcloud_z_max", 1.5);
    declare_parameter("pointcloud_range_max", 2.0);
  ...
  void loadParameters()
  {
    ...
    pipeline_config_.pointcloud.z_min =
      static_cast<float>(get_parameter("pointcloud_z_min").as_double());
    pipeline_config_.pointcloud.z_max =
      static_cast<float>(get_parameter("pointcloud_z_max").as_double());
    pipeline_config_.pointcloud.range_max =
      static_cast<float>(get_parameter("pointcloud_range_max").as_double());
  ...
  void timerCallback()
  {
    ...
    if (!pub_laserscan_ && !pub_pointcloud_ && !pub_self_filter_markers_) {
      return;
    }
    ...
    if (pub_laserscan_ && !hasAnyScanHits(output)) {
    ...
    const std::vector<rclcpp::Parameter> & parameters)
  {
    ...
    for (const auto & param : parameters) {
      ...
      } else if (stretch_core::isRobotSelfFilterParameter(name))
      {
        ...
        name == "pointcloud_z_min" || name == "pointcloud_z_max" ||
        name == "pointcloud_range_max")
      {
        loadParameters();
        pipeline_.setConfig(pipeline_config_);
  ...
  stretch_core::DualLidarHazardPipeline pipeline_;
  stretch_core::DualLidarHazardPipelineConfig pipeline_config_;
...
int main(int argc, char ** argv)
{
  ...
  auto node = std::make_shared<DualLidarHazardNode>();
