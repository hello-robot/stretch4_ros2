...
public:
    ...
    {   
        ...
        // Ransac
        this->declare_parameter<double>("plane_fitting_threshold", 0.1); 
        this->declare_parameter<double>("angle", 10.0); // can be futher tuned
        ...
        this->get_parameter("plane_fitting_threshold", plane_fitting_threshold);
        this->get_parameter("angle", angle_);
        ...
        const std::vector<rclcpp::Parameter> &parameters)
    {
        ...
        for (const auto &param : parameters) {
            if (param.get_name() == "z_min") {
                ...
                result.reason = "z_min is not being used with Ransac.";
...
private:
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr filter_floor_ransac(const pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud_in) {

        // -------------------------
        // Extract ROI for floor plane detection
        // -------------------------
        pcl::PointCloud<pcl::PointXYZ>::Ptr roi_cloud(new pcl::PointCloud<pcl::PointXYZ>());
        for (const auto& pt : cloud_in->points) {
            if (pt.z >= -0.4 && pt.z <= 0.1) { // adjustable
                roi_cloud->points.push_back(pt);
            }
        }
        if (roi_cloud->empty()) {
            RCLCPP_WARN(this->get_logger(), "No points in ROI for floor detection.");
            // return nullptr;
        }

        // -------------------------
        // Fit plane model to ROI
        ...
        // SACMODEL_PERPENDICULAR_PLANE worked better for the floor plane
        // seg.setModelType(pcl::SACMODEL_PLANE);
        
        seg.setModelType(pcl::SACMODEL_PERPENDICULAR_PLANE);
        seg.setAxis(Eigen::Vector3f(0.0, 0.0, 1.0));  // Z axis is "up"
        seg.setEpsAngle( angle_ * M_PI / 180.0f);

        // Like RANSAC but penalizes points far from the model instead of just counting inliers.
        seg.setMethodType(pcl::SAC_MSAC);
        // seg.setMethodType(pcl::SAC_RANSAC);

        seg.setDistanceThreshold(plane_fitting_threshold); // threshold that can be adjusted
        seg.setInputCloud(roi_cloud);
        seg.segment(*inliers, *coefficients);

        if (coefficients->values.size() < 4) {
            RCLCPP_ERROR(this->get_logger(), "Plane detection failed: insufficient coefficients (%zu found).", coefficients->values.size());
            return nullptr;
        }
        ...
        // -------------------------
        // Remove floor from full cloud based on plane distance
        // -------------------------
        pcl::PointCloud<pcl::PointXYZ>::Ptr no_floor_cloud(new pcl::PointCloud<pcl::PointXYZ>());
        int removed_points = 0;
        for (const auto& pt : cloud_in->points) {
            float dist = std::fabs(a*pt.x + b*pt.y + c*pt.z + d) / std::sqrt(a*a + b*b + c*c);
            if (dist > plane_fitting_threshold) {
                no_floor_cloud->points.push_back(pt);
            } else {
                removed_points++;
            }
        }
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr filter_region (const pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud_in, std::vector<float> & ranges) {
        ...
        for (const auto& point : cloud_in->points) {
            ...
            // if ( z < z_min_) continue; // floor points are not removed so no need for z_min
    ...
    {   
        ...
        *transformed_merged_cloud = *cloud_1 + *cloud_2;
        ...
        // filter ransac
        auto no_floor = filter_floor_ransac(transformed_merged_cloud);
        
        // filter 
        auto filter_region_pc = filter_region(no_floor, ranges);
